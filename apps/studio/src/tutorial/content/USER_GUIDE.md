...
**Help → Tutorial** (the "Help" button, always present — on the Startup Screen and inside an
open project) opens a real, bundled walkthrough of the current interface, entirely offline. It
offers to show itself once on first launch — dismissible with **Skip for Now** with no penalty
— and never blocks opening or recovering a project. It never touches your project's own
content: opening or closing it leaves whatever you were doing exactly as it was.

Restarting is always available (**Restart Tutorial**), and its final step links to this guide
and Troubleshooting, both bundled in the same offline viewer.

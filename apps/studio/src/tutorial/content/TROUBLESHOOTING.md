...
## I want to see the tutorial again / it's stuck on the wrong step

**Help → Tutorial** is always available, whether or not a project is open. **Restart
Tutorial** (inside the tutorial itself) always returns to the first step. Closing and
reopening the tutorial also starts over from the beginning — it never resumes mid-step, so
there's nothing to get "stuck" on. Skipping or completing it is remembered locally on this
machine only; there's no account or sync involved.
